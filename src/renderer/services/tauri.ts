...
const UDP_TRACKERS = [
  ...
  'udp://tracker.trackercenter.ovh:6969/announce',
  'udp://open.tracker.cl:1337/announce',
  'udp://open.demonii.com:1337/announce',
  'udp://open.tack.lol:6969/announce',
  'udp://tracker.0xcx.xyz:6969/announce',
  'udp://tracker.1-2-3.net:6969/announce',
  'udp://tracker.123456789.cf:6969/announce',
  'udp://tracker.123456789.pw:6969/announce',
  'udp://tracker.1c.pw:6969/announce',
  'udp://tracker.1trading.eu:6969/announce',
  'udp://tracker2.dler.org:80/announce',
  'udp://ipv6.rer.lol:2710/announce',
  'udp://bt.rer.lol:2710/announce',
  'udp://concen.org:6969/announce',
];
...
const HTTP_TRACKERS = [
  ...
  'http://tracker.ololosh.space:6969/announce',
  'http://explodie.org:6969/announce',
  'http://tracker.fnix.net:6969/announce',
];

const HTTPS_TRACKERS = [
  ...
  'https://tr.gxng.xyz:443/announce',
  'https://tr.kxmp.cf:443/announce',
  'https://tracker.waaa.moe:443/announce',
];
  ...
  torrent: {
    add: async (magnetUri: string, expectedSize?: number) => {
      ...
      if (enhancedMagnet.startsWith('magnet:?')) {
        const wsTradersParam = WS_TRACKERS.map(t => `&tr=${encodeURIComponent(t)}`).join('');
        if (!enhancedMagnet.includes('wss://')) {
          enhancedMagnet += wsTradersParam;
        }

        const hasUdpTracker = UDP_TRACKERS.some(t => enhancedMagnet.includes(t));
        if (!hasUdpTracker) {
          const udpTrackersParam = UDP_TRACKERS.map(t => `&tr=${encodeURIComponent(t)}`).join('');
          enhancedMagnet += udpTrackersParam;
        }

        const hasHttpTracker = HTTP_TRACKERS.some(t => enhancedMagnet.includes(t.replace('http://', '')));
        if (!hasHttpTracker) {
          const httpTrackersParam = HTTP_TRACKERS.map(t => `&tr=${encodeURIComponent(t)}`).join('');
          enhancedMagnet += httpTrackersParam;
        }

        const hasHttpsTracker = HTTPS_TRACKERS.some(t => enhancedMagnet.includes(t.replace('https://', '')));
        if (!hasHttpsTracker) {
          const httpsTrackersParam = HTTPS_TRACKERS.map(t => `&tr=${encodeURIComponent(t)}`).join('');
          enhancedMagnet += httpsTrackersParam;
        }
      }
